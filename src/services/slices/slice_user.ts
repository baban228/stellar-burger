import {
  ...
  export const fetch_user = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(handle_error(error, 'Ошибка при получении данных пользователя'));
      }
    }
  );
  ...
  export const fetch_update_user = createAsyncThunk(
    ...
    async (user_data: TRegisterData, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(handle_error(error, 'Ошибка при обновлении данных пользователя'));
      }
    }
  );
  ...
  export const fetch_logout = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        return rejectWithValue(handle_error(error, 'Ошибка при выходе из системы'));
      }
    }
  );
  ...
  export const user_selectors = {
    ...
    is_auth_checked_selector: (state: { user: TUser_State }) => state.user.is_auth_checked,
    is_loading_selector: (state: { user: TUser_State }) => state.user.is_loading,
    error_message_selector: (state: { user: TUser_State }) => state.user.error_message
  };
